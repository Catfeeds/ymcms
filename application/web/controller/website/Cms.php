<?php
namespace app\web\controller\website;
...
class Cms extends Common {
	...
	public function _initialize() {
    	...
    	$pageHead .= '<meta name="keywords" content="{$keywords}">';
    	$pageHead .= '<meta name="description" content="{$description}">';
    	$pageHead .= '<link rel="shortcut icon" href="{$sitedir}{$site.ico}" />';
		$pageHead .= '<link rel="stylesheet" href="__CSS__/page.css" />';
		$pageHead .= '</head>';
		$pageHead .= '<body>';
	...
	public function filetp($array) {
		...
		foreach ($array as $k) {
			...
			$info = $k->move(ROOT_PATH . 'public' . DS . 'upload' . DS . 'space' . DS . 'site_' . $this->site_id);
			if ($info) {
				/*图片成功上传后 获取上传信息*/
				$file_ico[] = $info->getFilename();
			}
		}
	...
	function uploadFile($file, $randclass) {
		...
		$destinationPath = $sitedir . $datedir;
		if (!file_exists($destinationPath)) {
			mkdir($destinationPath);
		}
		/*重命名*/
		$oldname = iconv('utf-8', 'gb2312', basename($files['name']));
		$fileName = mt_rand(10000000, 99999999) . '_' . $oldname;
	...
	public function channellist() {
		...
		$list = Db::name('channel')->where($where_list)->whereOr($whereOr_list)->paginate($pagelimit, false, $paginate);
		$lists = array();
		foreach ($list as $k => $v) {
			$lists[$k] = $v;
			/*状态*/
	...
	public function channeledit() {
		...
		if ($_POST) {
			...
			if (empty(input('name'))) {
				...
				/*判断结果集*/
				if ($result) {
					echo '<script>$(document).ready(function(){alertBox("' . $alert_success . '","' . url('website.cms/channellist') . '");})</script>';
				} else {
	...
	public function categoryedit() {
			...
			foreach ($content as $k => $v) {
				...
				if ($k == 'ico' && !empty($content['ico'])) {
					...
					foreach ($ico as $k2 => $v2) {
						$basename = explode('_', $v2);
						array_shift($basename);
						$basename = implode('_', $basename);
		...
		if ($_POST) {
			...
			if (empty(input('name'))) {
				...
				/*判断结果集*/
				if ($result) {
					echo '<script>$(document).ready(function(){alertBox("' . $alert_success . '","' . url('website.cms/categorylist') . '");})</script>';
				} else {
					echo '<script>$(document).ready(function(){alertBox("' . $alert_error . '","' . $_SERVER['HTTP_REFERER'] . '");})</script>';
				}
			}
	...
	public function contentedit() {
		...
		if ($_POST) {
			...
			if (empty(input('name'))) {
				...
				/*判断结果集*/
				if ($result) {
					echo '<script>$(document).ready(function(){alertBox("' . $alert_success . '","' . url('website.cms/contentlist') . '");})</script>';
				} else {
					echo '<script>$(document).ready(function(){alertBox("' . $alert_error . '","' . $_SERVER['HTTP_REFERER'] . '");})</script>';
				}
			}
	...
	public function blockedit() {
		...
		if ($id) {
			...
			$s_url = url('website.cms/tempfit').'?id='.$temp_id;
			/*编辑*/ 
			/*查询参数*/
			$where['id'] = $id;
			...
			if (!empty($content['type'])) {
				if (!empty($content['content'])) {
					/*列表菜单*/
					if ($content['type'] == 2){
						$content_con = $this->myTrim($content['content']);
		...
		if ($_POST) {
			...
			if (input('type') == 1 || input('type') == 4 || input('type') == -1 || input('type') == -2) {
				...
				/*列表菜单*/
				$content = $this->myTrim(input('content2'));
			} else if (input('type') == 3) {
				/*自定义菜单*/
				$content = $this->myTrim(input('content3'));
			}
			/*验证参数*/
			if (empty(input('name'))) {
				echo '<script>$(document).ready(function(){alertBox("名称不可为空..","' . $_SERVER['HTTP_REFERER'] . '");})</script>';
				...
				if ($id) {
					...
					$data['temp_id'] = $site_info['temp_id']; /*模板ID*/
					$data['plugin'] = 0; /*用户模板*/
					$data['addtime'] = $nowtime; /*添加时间*/
					/*插入数据*/
	...
	public function tempfit(){
		...
		$html .= '</ul>';
		$html .= '<div class="black"><a href="'.url('website.cms/tempmy').'"><img src="__IMAGE__/ico_white/arrow_left.png" />返回</a></div>';
		...
		$html .=		'$.post("{:url(\'website.cms/tempfitBlockClone\')}",{';
		...
		$html .=		'$.post("{:url(\'website.cms/tempfitBlockSave\')}",{';
		$html .=			'block_id : block_id,';	/*模块ID*/
		$html .=			'top : top,';/*上*/
		$html .=			'left : left,';/*左*/
		$html .=			'width : width,';/*宽*/						
	...
	public function groupedit() {
		...
		if ($_POST) {
			...
			if (empty(input('name'))) {
				...
				/*判断结果集*/
				if ($result) {
					echo '<script>$(document).ready(function(){alertBox("' . $alert_success . '","' . url('website.cms/grouplist') . '");})</script>';
				} else {
					echo '<script>$(document).ready(function(){alertBox("' . $alert_error . '","' . $_SERVER['HTTP_REFERER'] . '");})</script>';
				}
			}
	...
	public function adminlist() {
		...
		$whereOr_user_list2['site_ids'] = array('like', $this->site_id . ',%'); /*管理组：站点员工*/
		$whereOr_user_list3['site_ids'] = array('like', '%,' . $this->site_id . ',%'); /*管理组：站点员工*/
		$whereOr_user_list4['site_ids'] = array('like', '%,' . $this->site_id); /*管理组：站点员工*/
		/*分页参数*/
		$paginate = array('query' => array( /*url额外参数*/
		'limit' => $limit, /*每页条数*/
		'search' => $search, /*搜索*/
	...
	public function adminedit() {
		...
		if ($_POST) {
			...
			if (empty(input('name'))) {
				...
				/*判断结果集*/
				if ($result) {
					echo '<script>$(document).ready(function(){alertBox("' . $alert_success . '","' . url('website.cms/adminlist') . '");})</script>';
				} else {
					echo '<script>$(document).ready(function(){alertBox("' . $alert_error . '","' . $_SERVER['HTTP_REFERER'] . '");})</script>';
				}
			}
	...
	public function useredit() {
		...
		if ($_POST) {
			...
			if (empty(input('name'))) {
				...
				/*判断结果集*/
				if ($result) {
					echo '<script>$(document).ready(function(){alertBox("' . $alert_success . '","' . url('website.cms/userlist') . '");})</script>';
				} else {
					echo '<script>$(document).ready(function(){alertBox("' . $alert_error . '","' . $_SERVER['HTTP_REFERER'] . '");})</script>';
				}
			}
	...
	public function orderedit() {
		...
		if ($_POST) {
			...
			if (empty(input('order'))) {
				...
				/*判断结果集*/
				if ($result) {
					echo '<script>$(document).ready(function(){alertBox("' . $alert_success . '","' . url('website.cms/orderlist') . '");})</script>';
				} else {
					echo '<script>$(document).ready(function(){alertBox("' . $alert_error . '","' . $_SERVER['HTTP_REFERER'] . '");})</script>';
				}
			}
	...
	public function commentedit() {
		...
		if ($_POST) {
			...
			if (empty(input('order'))) {
				...
				/*判断结果集*/
				if ($result) {
					echo '<script>$(document).ready(function(){alertBox("' . $alert_success . '","' . url('website.cms/commentlist') . '");})</script>';
				} else {
					echo '<script>$(document).ready(function(){alertBox("' . $alert_error . '","' . $_SERVER['HTTP_REFERER'] . '");})</script>';
				}
			}
	...
	public function msg() {
		...
		if ($_POST) {
			...
			$request->setPhoneNumbers($data['phone']);//必填-短信接收号码
			$request->setSignName("奥站");//必填-短信签名
			//必填-短信模板Code
			$request->setTemplateCode("SMS_76545064");
			//选填-假如模板中存在变量需要替换则为必填(JSON格式)
			$request->setTemplateParam("{\"regcode\":\"".$data['content']."\"}");//短信签名内容:
			//选填-发送短信流水号
